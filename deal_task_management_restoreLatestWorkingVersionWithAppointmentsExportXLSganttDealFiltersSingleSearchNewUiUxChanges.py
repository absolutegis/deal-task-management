...
def generate_gantt_chart(deal_name, deal, filtered_tasks_df):
    current_date = pd.Timestamp(datetime.now().date())  # Ensure current_date is a pandas Timestamp
    ...
    key_dates = {
        'GF Submittal Date': pd.to_datetime(deal['GF Submittal Date'], errors='coerce'),
        'Green Folder Meeting Date': pd.to_datetime(deal['Green Folder Meeting Date'], errors='coerce'),
        'IP Expiration Date': pd.to_datetime(deal['IP Expiration Date'], errors='coerce'),
        'Projected Deal First Closing Date': pd.to_datetime(deal['Projected Deal First Closing Date'], errors='coerce'),
    }
    ...
    if not gantt_df.empty:
        ...
        fig.update_layout(
            xaxis=dict(
                ...
                tickformat="%m/%d/%Y"  # Format the x-axis ticks for MM/DD/YYYY
            ),
